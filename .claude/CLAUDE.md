...
## Token Management (Token Economy)

### Session Initialization
**Every session should start in Haiku mode** for maximum economy:
```bash
# Dentro do Claude Code
/model claude-haiku-4-5-20251001

# Depois, inicializar hooks
source .claude/hooks/token-economy-init.sh
```

### Model Selection Strategy
| Task | Model | Cost Ratio |
|------|-------|-----------|
| **Exploração, busca, leitura** | haiku | 1x (base) |
| **Coding simples, refatoração** | haiku | 1x |
| **Implementação de features** | sonnet | 12x |
| **Decisões arquiteturais** | opus | 60x |

### Economizar Tokens: Regras Práticas

1. **Ler parcial, não inteiro**
   - ❌ "Leia X.js" (lê arquivo inteiro)
   - ✅ "Leia linhas 50-100 de X.js" (lê só o necessário)

2. **Usar `/compact` agressivamente**
   - Depois de resolver um bug: `/compact`
   - Antes de começar feature nova: `/compact`
   - Conversa com > 100 mensagens: `/compact`

3. **Delegar via Agent com modelo certo**
   - Busca de padrão → agent com `model: "haiku"`
   - Implementação → agent com `model: "sonnet"`
   - Nunca deixar agent decidir modelo — sempre especificar

4. **Ignorar pesados**
   - `.claudeignore` bloqueia node_modules, dist, build
   - Nunca deixar Claude ler lock files
   - Remover logs antigos antes de sessions

5. **Monitorar gasto**
   ```bash
   /usage  # Ver quanto já gastou
   /budget # Ver limite mensal
   ```

### Checklist Pré-Session
- [ ] `/model haiku` ativado
- [ ] `source .claude/hooks/token-economy-init.sh` rodado
- [ ] `.claudeignore` presente e atualizado
- [ ] Arquivos de teste/build removidos localmente
- [ ] Contexto CLAUDE.md <= 300 linhas (se > 300, mover para rules/)
