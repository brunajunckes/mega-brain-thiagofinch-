...
export async function GET(request, { params }) {
  try {
    ...
    const r = await fetch(`${BACKEND}/api/projects/${id}/craft-standards`, {
    ...
    try {
      ...
      // If backend doesn't have a GET, return defaults from the global endpoint
      const fallback = await fetch(`${BACKEND}/api/novel-process/craft-standards`);
      const fbText = await fallback.text();
      try {
        JSON.parse(fbText);
        return new Response(fbText, { status: fallback.status, headers: { 'Content-Type': 'application/json' } });
      } catch {
