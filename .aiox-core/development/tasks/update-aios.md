...
bash .aiox-core/scripts/update-aios.sh

# Review changes shown by the script, then:
git add .aiox-core && git commit -m "chore: sync AIOS framework"   # Apply changes
# OR
...
execution:
  script: .aiox-core/scripts/update-aios.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .aiox-core/scripts/update-aios.sh
  3. Review changes displayed
  4. git add .aiox-core && git commit -m "chore: sync AIOS framework"  # to apply
