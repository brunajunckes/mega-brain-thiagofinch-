...
export async function askCouncilCommand(input: {
  question: string;
  domains?: string[];
}): Promise<{
  success: boolean;
  ...
  try {
    ...
    // Format response
    const argumentsList = Array.from(deliberation.arguments.values()).map((arg) => ({
      agent: arg.agentId,
    ...
    const votes = Array.from(deliberation.votes.entries()).map(([memberId, vote]) => ({
      member: memberId,
    ...
    return {
      success: false,
      message: `❌ Deliberation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
    };
  }
}
 ...
 */
export async function getCouncilComposition(): Promise<{
  success: boolean;
  members?: any;
  message: string;
}> {
  try {
    ...
    return {
      success: false,
      message: `❌ Failed to get composition: ${error instanceof Error ? error.message : 'Unknown error'}`,
    };
  }
}
